library dart_and_cpp_classes.build_cpp_extension;

import 'dart:io';
import 'package:ccompilers/ccompilers.dart';

class CppExtensionProject {
  ...
  List _compilerInput = ['cpp_extension.cc'];
  List _linkerInput = ['cpp_extension'];
  List _linkerLibpath = [];
  String _linkerOutput = 'cpp_extension';
  List _unusedFileExtensions = ['exp', 'lib', 'o', 'obj'];

  CppExtensionProject(String pathToProject) {
  ...
  int _build() {
    ...
    var compile = new CommandLineTask(() => _compiler.run(_compilerArgs.arguments, workingDirectory : _pathToProject));
    compile.before = 'Compile...';
    compile.success = 'Compilation succesful.';
    compile.fail = 'Compilation failed.';
    tasks.add(compile);
    var link = new CommandLineTask(() => _linker.run(_linkerArgs.arguments, workingDirectory : _pathToProject));
    link.before = 'Link...';
    ...
    _clean(_pathToProject, _unusedFileExtensions);
    return 0;
  }
